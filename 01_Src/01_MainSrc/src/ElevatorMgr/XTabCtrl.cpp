...
void CXTabCtrl::OnSelchange(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	else
	{
		...
		//发送消息给父窗口，当前切换了窗口。需要更新父窗口的内容
		iNewTab = GetCurSel();
		::SendMessage(pWnd->m_hWnd,USER_SELCHANGE_MSG,iNewTab,0);
		...
		if (pOwner && IsWindow(pOwner->m_hWnd))
		{
			...
			hr.code = TCN_SELCHANGE;
			hr.idFrom = GetDlgCtrlID();
			hr.hwndFrom = pOwner->m_hWnd;
			pOwner->SendMessage(WM_NOTIFY,hr.idFrom,(LPARAM)&hr);
		}
	}
...
{	
	...
	//zhanghaiwei for bug 0,to alternate caption text displaying position
	//2008.11.20
